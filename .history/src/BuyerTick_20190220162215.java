			...
			if (msg != null){
				...
				if(msg.getPerformative()==ACLMessage.INFORM){
					...
					if(obj==null || ((Product)obj).sold ){
						...
						System.out.println("[~]"+ MeAgent.getLocalName()+": "+msg.getSender().getLocalName()+" sais he has such a product with price "+p.price+" and will end in "+MeAgent.calculateRemaindTime(p.time_end)+" seconds");
						...
						if(MeAgent.ProductExists(p.id,msg.getSender().getLocalName())){
							...
							if(MeAgent.budget-p.bidStep>=(p.price)){
								...
								double toBid=b_v*propability;
								currProduct.probability_average.add(toBid);
								
								double sum=0;
								for(double v:currProduct.probability_average){
									sum+=v;
								}
								double ave=sum/currProduct.probability_average.size();
